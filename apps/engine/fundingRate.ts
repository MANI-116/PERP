import { OrderBook, Position, PriceLevelObject, Node } from "@repo/engine-package";
...
export function fundingRateEngine(orderBook: OrderBook, rate: bigint, direction: -1n | 1n) {
  ...
  orderBook.shorts.forEach((levelObject) => {
    ...
    if (position) applyFundingRate(position.value, rate, direction, newShorts, newLongs);
  });
...
function applyFundingRate(
  ...
  rate: bigint,
  direction: -1n | 1n,
  shorts: Map<bigint, PriceLevelObject<Position>>,
  longs: Map<bigint, PriceLevelObject<Position>>,
) {
  position.initialMargin +=
    position.side === "SHORT"
      ? (position.qty * position.avgPrice * rate * direction) / 1000n
      : (position.qty * position.avgPrice * rate * direction * -1n) / 1000n;
  ...
  if (level === undefined) {
    level = PriceLevelObject.createFromOrder<Position>(position);
