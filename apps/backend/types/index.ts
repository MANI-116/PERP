...
export class Position {
  ...
  setLiquidationPrice() {
    if (this.qty === 0n || this.state === 'CLOSED') {
      throw new Error(
        'qty is zero,we cannot measure of zero qty or state is closed ',
      );
    }
    let direction = this.side === 'SHORT' ? 1n : -1n;
    this.liquidationPrice =
      (this.avgPrice * this.qty + this.initialMargin * direction) /
      (this.qty * (1000n - this.market.mmr));
  }
...
export class Dll<T> {
  ...
  removeNode(node: Node<T>): { success: boolean; message: string } {
    ...
    if (this.tail === node) {
      ...
      if (leftNode === null)
        return { success: false, message: 'single node,remove the list' };
      this.tail = leftNode;
      leftNode.right = null;
      node.left = null;
      this.length--;
    ...
    if (!leftNode || !rightNode)
      return { success: false, message: 'unable to remove node' };
...
export class AskTree {
  ...
  private findPosition(price: bigint, start: number, end: number): number {
    if (
      start > end ||
      start >= this.prices.length ||
      end >= this.prices.length ||
      start < 0 ||
      end < 0
    ) {
      return -1;
...
export class OrderBook {
  ...
  removeShort(position: Position, lp?: bigint) {
    ...
    const level = lp
      ? this.shorts.get(lp)
      : this.shorts.get(position.liquidationPrice);
    if (level === undefined) {
  ...
  removeLong(position: Position, lp?: bigint) {
    ...
    const level = lp
      ? this.longs.get(lp)
      : this.longs.get(position.liquidationPrice);
    if (level === undefined) {
      return { success: false, message: 'position doesnot exist' };
    }
    //get the reference of the position
