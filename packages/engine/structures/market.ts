    ...
    calculateEstimatedPrice(qty:bigint,side:OrderSide){
            ...
            for(let i =0; i< totalLevels;i++ ){
                ...
                if(availableQty >= requiredQty){
                    ...
                    side === "SHORT"? this.orderbook.bidTree.removePrice(priceLevel): this.orderbook.askTree.removePrice(priceLevel);
                }
    
            }
            removedPrices.forEach((p)=> side === "SHORT"? this.orderbook.bidTree.addPrice(p): this.orderbook.askTree.addPrice(p))
            if(filled === 0n){
                return 0n;
            }
